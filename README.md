...
模型只提出術語候選；候選會在網頁中顯示成可編輯表格。產生翻譯初稿時，
程式會把目前表格自動合併至 Markdown 術語表。第一版的目標是讓完整工作
流程可執行，不進行模型比較。
...
- 產生翻譯時自動採用目前候選表格，不需另按批准
- 長文先依段落、句子及最大字數建立語境區塊；每批最多處理 8 筆候選
- 翻譯依原文長度動態限制輸出 tokens 與單次等待時間
- 狀態列分別顯示初稿與重試階段耗時
- 明確的英文術語殘留由程式直接替換，避免額外呼叫模型
- 以網頁表格審核術語候選
- 依 `glossary.md` 分段翻譯
...
6. 按「產生翻譯初稿」；程式會自動更新畫面中的 `glossary.md`。
7. 檢閱初稿後，按「執行品質審查」。
8. 人工確認審查稿，再匯出譯文與術語表。
